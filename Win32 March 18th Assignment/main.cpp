...
public:
    ...
    virtual void Render() override
    {
        ...
        {
            currentVertices[i].x = (currentVertices[i].x * m_owner->transform.scaleX) + m_owner->transform.x;
            currentVertices[i].y = (currentVertices[i].y * m_owner->transform.scaleY) + m_owner->transform.y;
        }
...
public:
    virtual void Update(float dt) override
    {
        ...
        if (g_keys['W']) m_owner->transform.y += moveAmount;
        if (g_keys['S']) m_owner->transform.y -= moveAmount;
        if (g_keys['A']) m_owner->transform.x -= moveAmount;
        if (g_keys['D']) m_owner->transform.x += moveAmount;
...
private:
    void ClampPosition()
    {
        if (m_owner->transform.x > 0.8f) m_owner->transform.x = 0.8f;
        if (m_owner->transform.x < -0.8f) m_owner->transform.x = -0.8f;
        if (m_owner->transform.y > 0.8f) m_owner->transform.y = 0.8f;
        if (m_owner->transform.y < -0.8f) m_owner->transform.y = -0.8f;
    }
};
...
public:
    virtual void Update(float dt) override
    {
        ...
        if (g_keys[VK_UP])    m_owner->transform.y += moveAmount;
        if (g_keys[VK_DOWN])  m_owner->transform.y -= moveAmount;
        if (g_keys[VK_LEFT])  m_owner->transform.x -= moveAmount;
        if (g_keys[VK_RIGHT]) m_owner->transform.x += moveAmount;
...
bool GameLoop::Initialize(HWND hWnd)
{
    ...
    obj1->transform.x = -0.5f;
    obj1->transform.y = 0.0f;
    ...
    obj2->transform.x = 0.5f;
    obj2->transform.y = 0.0f;
